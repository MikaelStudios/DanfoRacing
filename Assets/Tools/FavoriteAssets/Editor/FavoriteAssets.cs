...
namespace KrisFavoriteAssets
{
    public class FavoriteAssets : EditorWindow
    {
        ...
        [MenuItem("Tools/Quick Hack")]
        public static void QuickHack()
        {
            foreach (GameObject selected in Selection.gameObjects)
            {
                //Debug.Log(selected.name);
                foreach (Transform eachChild in selected.GetComponentsInChildren<Transform>())
                {
                    if (eachChild.name == "CheckOtherCarCollision")
                    {
                        //Debug.Log("Child found  in " + );

                        eachChild.tag = "Untagged";
                        Debug.Log(selected.name + " changed tag");
                        return;
                    }
                }
                Debug.LogWarning("not found under " + selected.name);
            }
        }
        ...
        public void OnGUI()
        {
            ...
            foreach (AssetData assetData in assetsData.assets)
            {
                ...
                //GUIContent g =new GUIContent (" " + assetData.name);
                GUIStyle gUIStyle = new GUIStyle(GUI.skin.button);
                gUIStyle.alignment = TextAnchor.MiddleLeft;
                if (GUILayout.Button(new GUIContent(" " + assetData.name, AssetDatabase.GetCachedIcon(assetData.path)), gUIStyle, GUILayout.Height(18)))
                {
